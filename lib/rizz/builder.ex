defmodule Rizz.Builder do
  ...
  def to_xml(feed) do
    ...
    Saxy.encode!(rss_element, version: "1.0", encoding: "UTF-8")
  end
  ...
  defp build_item_element(item) do
    ...
    json_ld_element = if item.json_ld do
  ...
  defp build_element(name, value) when is_binary(value) or is_integer(value), do: {name, [], [to_string(value)]}
  defp build_element(_name, _value), do: nil

  defp build_ai_element(_name, nil), do: nil
  defp build_ai_element(name, value) when is_binary(value) or is_integer(value), do: {"ai:" <> name, [], [to_string(value)]}
  defp build_ai_element(_name, _value), do: nil

  defp format_datetime(nil), do: nil
  defp format_datetime(%DateTime{} = dt), do: Timex.format!(dt, "{RFC822}")
